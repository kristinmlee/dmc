...
det_FOmegas_mig_e = readRDS("det_FOmegas_mig_e.killifish.RDS")

inv_FOmegas_mig_e = readRDS("inv_FOmegas_mig_e.killifish.RDS")
...
for(j in 1 : ncol(distances)) {
	...
	saveRDS(compLikelihood_mig, paste("compLikelihood_mig_selSites_", paste(j, collapse = "_"), ".killi.RDS", sep = ""))
}