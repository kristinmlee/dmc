...
det_FOmegas_mixed.migInd_e = readRDS("detFOmegas_mixed.migInd.RDS")

inv_FOmegas_mixed.migInd_e = readRDS("invFOmegas_mixed.migInd.RDS")
...
compLikelihood_mixed.migInd = lapply(1 : ncol(distances), function(j) {
	lapply(1 : length(sels), function(sel) {
		lapply(1 : length(gs[1]), function(g) {
			lapply(1 : length(times[1]), function(time) {
				lapply(1 : length(migs), function(mig) {
					lapply(1 : length(sources), function(my.source) {
						calcCompLikelihood_bin.5par(j, det_FOmegas_mixed.migInd_e, inv_FOmegas_mixed.migInd_e, sel, g, time, mig, my.source)
					})
				})
			})
		})
...
saveRDS(compLikelihood_mixed.migInd, paste("compLikelihood_mixed.migInd.killi.RDS", sep = ""))