...
det_FOmegas_mixed.svInd_1_e = readRDS("detFOmegas_mixed.svInd_1.RDS")

inv_FOmegas_mixed.svInd_1_e = readRDS("invFOmegas_mixed.svInd_1.RDS")
...
calcLikelihood_bin.5par = function(site, j, det_FOmegas, inv_FOmegas, par1, par2, par3, par4, par5) {
	bin = distBins[site, j]
	my.x = as.matrix(freqs_MC[ ,site])
	my.e = epsilons[site]*(1-epsilons[site])

	likelihood = 1/(sqrt((2*pi)^k*(det_FOmegas[[par1]][[par2]][[par3]][[par4]][[par5]][[bin]]*my.e^rank))) * exp(-1/2*t(my.x-mu) %*% (inv_FOmegas[[par1]][[par2]][[par3]][[par4]][[par5]][[bin]]/my.e) %*% (my.x-mu))
	
	return(log(likelihood))
}

calcCompLikelihood_bin.5par = function(j, det_FOmegas, inv_FOmegas, par1, par2, par3, par4, par5) {
	all = sapply(1:nrow(distances), {
			function(i) calcLikelihood_bin.5par(i, j, det_FOmegas, inv_FOmegas, par1, par2, par3, par4, par5)
		})
	return(sum(all))
}

for(j in 30:1) {
	compLikelihood_mixed.svInd_1 = lapply(1 : length(sels), function(sel) {
		lapply(1: length(gs), function(g) {
			lapply(1 : length(times), function(time) {
				lapply(1 : length(migs[1]), function(mig) {
					lapply(1 : length(sources), function(my.source) {
						calcCompLikelihood_bin.5par(j, det_FOmegas_mixed.svInd_1_e, inv_FOmegas_mixed.svInd_1_e, sel, g, time, mig, my.source)
					})
				})
			})
		})
	})
	saveRDS(compLikelihood_mixed.svInd_1, paste("compLikelihood_mixed.svInd_source", sources, "_selSite", j, ".killi.RDS", sep = ""))
}