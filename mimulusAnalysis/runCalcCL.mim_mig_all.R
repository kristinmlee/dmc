...
det_FOmegas_mig_e = readRDS("det_FOmegas_mig_e.mim.RDS")

inv_FOmegas_mig_e = readRDS("inv_FOmegas_mig_e.mim.RDS")
...
compLikelihood_mig = lapply(1 : ncol(distances), function(j) {
	lapply(1 : length(sels), function(sel) {
		lapply(1 : length(migs), function(mig) {
			lapply(1 : length(sources), function(source) {
				calcCompLikelihood_bin.3par(j, det_FOmegas_mig_e, inv_FOmegas_mig_e, sel, mig, source)
			})
		})
	})
})

saveRDS(compLikelihood_mig, paste("compLikelihood_mig_selSites_", paste(selSiteInterest, collapse = "_"), ".mim.RDS", sep = ""))