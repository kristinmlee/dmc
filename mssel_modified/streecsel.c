...
*
*	This code has been modified by Kristin M Lee (krmlee@ucdavis.edu)
*	to allow for the independent origin of multiple beneficial lineages
*	(Feb 2016)
*
**************************************************************************/
	...
	while( nchrom > 1 ) {
		...
		for( pop1 = 0 ; pop1 < npop; pop1++){
			...
			if( ( freqder[pop1] == 0.0 ) && ( config[1][pop1] > 0 ) ){
				...
				else{ //KL allow multiple independent origins of beneficial allele
					config[1][pop1]--;
					config[0][pop1]++;
					for( ic=0; ic<nchrom; ic++){
		        		if( ( chrom[ic].pop == pop1 ) && ( chrom[ic].allele == 1 ) ) {
							chrom[ic].allele = 0; 
							break;
		        		}
					}
		...
		/* end addition */
		   ...
		   gotmigranti:
			...
			/* pick the two, c1, c2  */
			   ...
			   //fprintf(stderr,"coal %lf %d %d %d\n", t,callele,cpop,dec); //test
